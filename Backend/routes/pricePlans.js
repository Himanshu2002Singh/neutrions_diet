...
async function getAll(req, res) {
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to fetch price plans', error: error.message });
  }
}
...
async function getById(req, res) {
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to fetch price plan', error: error.message });
  }
}
...
async function create(req, res) {
  try {
    ...
    res.status(201).json({ success: true, message: 'Price plan created successfully', data: plan });
  } catch (error) {
    console.error('Error creating price plan:', error);
    res.status(500).json({ success: false, message: 'Failed to create price plan', error: error.message });
  }
}
...
async function update(req, res) {
  try {
    ...
    res.json({ success: true, message: 'Price plan updated successfully', data: plan });
  } catch (error) {
    console.error('Error updating price plan:', error);
    res.status(500).json({ success: false, message: 'Failed to update price plan', error: error.message });
  }
}
...
async function deletePlan(req, res) {
  try {
    ...
    res.json({ success: true, message: 'Price plan deleted successfully' });
  } catch (error) {
    console.error('Error deleting price plan:', error);
    res.status(500).json({ success: false, message: 'Failed to delete price plan', error: error.message });
  }
}
...
async function reorder(req, res) {
  try {
    ...
    for (let i = 0; i < planIds.length; i++) {
      await PricePlan.update({ sortOrder: i + 1 }, { where: { id: planIds[i] } });
    }
    res.json({ success: true, message: 'Price plans reordered successfully' });
  } catch (error) {
    console.error('Error reordering price plans:', error);
    res.status(500).json({ success: false, message: 'Failed to reorder price plans', error: error.message });
  }
}
