...
import personalizedHealthImage from '../../assets/personalized_health.png';

interface FarmFreshSectionProps {
  onStartJourneyClick?: () => void;
}

export function FarmFreshSection({ onStartJourneyClick }: FarmFreshSectionProps) {
  return (
    <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 mb-12 items-center">
      <div className="order-2 lg:order-1">
        <p className="text-sm mb-4 opacity-70">AI-Powered WorkOut plan</p>
        <h2 className="text-3xl lg:text-5xl font-bold mb-6 leading-tight">
          Personalized Work Out with
          <span className="text-[#FF6B4A]"> AI Coach</span>
          <br />
          <span className="text-[#FF6B4A]"></span>
        </h2>
        <p className="text-gray-600 mb-8 leading-relaxed">
          Get Workout recommendations based on your medical conditions, body type, and goals. 
          Our AI Coach Ria provides personalized workout suggestions, calorie goals, and motivational reminders 
          to keep you on track with your health journey.
        </p>
        <div className="flex flex-col sm:flex-row gap-4">
          <button 
            onClick={onStartJourneyClick}
            className="bg-[#FF6B4A] text-white px-6 py-3 rounded-lg flex items-center gap-2 hover:opacity-90 transition-opacity"
          >
            Start Your Journey
