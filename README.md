...
const createRateLimiter = (windowMs, max, message) => {
  return rateLimit({
    ...
    message: {
      success: false,
      message: message || 'Too many requests from this IP, please try again latee
r.'
    },
    ...
    handler: (req, res) => {
      res.status(429).json({
        success: false,
        message: message || 'Too many requests from this IP, please try again laa
ter.',
const express = require('express');
const cors = require('cors');
const helmet = require('helmet');
...
const generalLimiter = createRateLimiter(
  ...
  parseInt(process.env.RATE_LIMIT_MAX_REQUESTS) || 100, // limit each IP to 100  
...
const errorHandler = (err, req, res, next) => {
  ...
  if (err.name === 'SequelizeConnectionError' || err.name === 'SequelizeConnectii
onRefusedError') {
...
const errorHandler = (err, req, res, next) => {
  ...
  if (err.name === 'SequelizeDatabaseError') {
    ...
    return res.status(500).json({
      ...
      error: process.env.NODE_ENV === 'development' ? err.message : 'An error occ
curred while processing your request'
    });
  }
